...
struct App {
    ...
    mode: Mode,
    script_name: String,
    should_quit: bool,  // Добавлен флаг для выхода
    selected_output: Option<String>,  // Сохраняем выбранную строку
}
...
fn handle_selection_mode(app: &mut App, key: KeyEvent) -> Result<(), Box<dyn std::error::Error>> {
    match key.code {
        ...
        KeyCode::Enter => {
            // Сохраняем выбранную строку и выходим
        ...
        KeyCode::Esc => {
            ...
            app.mode = Mode::Input;
            // Очищаем вывод скрипта при возврате
...
fn ui(frame: &mut Frame, app: &App) {
    ...
    if !app.script_output.is_empty() {
        ...
        // Это может произойти, если скрипт ничего не вывел
